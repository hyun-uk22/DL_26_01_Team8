...
이 프로젝트는 기본적으로 `yolo26x-pose.pt` 모델 파일을 사용합니다. 모델 가중치(`*.pt`)는 용량이 커서 Git에 포함하지 않습니다. 실행 전에 프로젝트 루트에 모델 파일을 준비하세요.
...
3. 필요한 경우 영상 위에서 마우스로 crop 영역을 지정합니다.
4. `포즈 추출`을 눌러 레퍼런스 keypoint를 추출합니다.
5. `분석 시작`으로 실시간 카메라 자세를 비교하거나, `카메라 녹화 시작` 후 녹화 구간을 분석합니다.
...
현재 `.gitignore` 기준으로 아래 파일/폴더는 Git에 올리지 않습니다.

- `.venv/`
- `__pycache__/`
- `dataset/`, `dataset_split/`, `dummy_data/`, `data_preprocess/`
- `*.pt` 모델 가중치
- `data.yaml`
- 로컬 문서 및 임시 파일

따라서 GitHub에 업로드한 뒤 다른 환경에서 실행하려면 모델 가중치와 필요한 데이터 파일은 별도로 준비해야 합니다.
