...
- EMA + One Euro Filter 기반 keypoint temporal smoothing
- 사용자 카메라 실시간 자세 분석
...
- 웹캠
- YOLO pose 모델 가중치 파일
- GPU 권장 (Windows: NVIDIA CUDA, macOS: Apple Silicon MPS)
...
# CUDA 버전 PyTorch (NVIDIA GPU)
pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu126

# 나머지 의존성
pip install av opencv-python pillow numpy ultralytics yt-dlp pyyaml requests scipy polars psutil matplotlib
```
...
```bash
source .venv/bin/activate
pip install torch torchvision torchaudio --index-url https://download.pytorch.org/whl/cu126  # CUDA
# 또는
...
`pushup_videos` 폴더에 mp4 팔굽혀펴기 영상을 넣은 뒤 다음 명령으로 Procrustes, DTW, EMA + One Euro Filter 적용 전후를 pairwise로 평가할 수 있습니다.

```powershell
.\.venv\Scripts\python.exe benchmarks\pushup_benchmark.py
```

결과는 `benchmark_outputs/pushup` 아래 CSV로 저장됩니다. 자세한 입력 형식과 지표 의미는 `benchmarks/README_pushup_benchmark.md`를 참고하세요.
...
- 필터링: 레퍼런스, 실시간 카메라, 녹화본 분석 시퀀스별로 EMA와 One Euro Filter를 적용해 프레임 간 keypoint 떨림을 줄입니다.
